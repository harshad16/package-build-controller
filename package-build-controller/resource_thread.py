...
def process_taskq(task_q, global_count, object_map):
    ...
    if q_resource_kind == "BuildConfig" and not retrigger:
        ...
        build_created, build_response = get_buildconfig(req_url=OCP_URL,
                                                        req_headers=HEADERS,
                                                        namespace=NAMESPACE,
                                                        build_config_name=q_resource_name)
        if not build_created:
            created = create_buildconfig(req_url=OCP_URL,
                                                     req_headers=HEADERS,
                                                     namespace=NAMESPACE,
            ...
            bexist , bresp = get_build(req_url=OCP_URL, req_headers=HEADERS, namespace=NAMESPACE,
            ...
            if bexist:
                ...
                if phase == "Complete":
                    ...
                    job_created, job_response = get_job(req_url=OCP_URL, req_headers=HEADERS, namespace=NAMESPACE,
                                                             job_name=job_name)
                    ...
                    if not job_created:
                        ...
                        if job_status == "BackoffLimitExceeded":
                            ...
                            dstate, dresp = delete_job(req_url=OCP_URL,
                                       req_headers=HEADERS,
                                       namespace=NAMESPACE,
                                       job_name=job_name)
    ...
    # ==========================
        ...
        if not get_imagestream(req_url=OCP_URL,
                               req_headers=HEADERS,
                               namespace=NAMESPACE,
                               imagestream_name=q_resource_name):
            created = create_imagestream(req_url=OCP_URL, req_headers=HEADERS, namespace=NAMESPACE,
    ...
    # ==========================
        ...
        job_status = get_job_status(job_response["status"])
        if not job_created:
            created = create_job(req_url=OCP_URL, req_headers=HEADERS, namespace=NAMESPACE, job_name=q_resource)
            if not created:
