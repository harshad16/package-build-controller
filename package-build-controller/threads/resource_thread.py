...
def process_taskq(task_q, global_count, object_map):
    ...
    check_bkey_exists = (
        lambda mydict, mykey: mydict[mykey] if mykey in mydict else False
    )
    q_item = task_q.get()
    ...
    logging.debug(
        "{} processing STARTED. Q-size: {} , G:{}.".format(
    ...
    if q_resource_kind == "BuildConfig" and not retrigger:
        ...
        if not build_created:
            ...
            if not created:
                raise Exception(
                    "Build could not be created for {}".format(q_resource_name)
                )
            ...
            logging.debug(
                "{} The Build {} exists={} version={}. G:{}".format(
                    task_q.qsize(),
                    q_resource_name,
                    bexist,
            ...
            if bexist:
                ...
                if phase == "Complete":
                    ...
                    if not job_created:
                        ...
                        global_count.increment()
                        logging.debug(
                            "{} processing DONE; ADDED new task: {}; kind: {} G:{}".format(
                                task_q.qsize(), job_name, "Job", global_count
                            )
                        )
                        ...
                        if job_status == "BackoffLimitExceeded":
                            # Delete the Job
                            logging.debug(
                                "{} processing DONE; deleting existing {}; ".format(
                                    task_q.qsize(), job_name
                                )
                            )
                            ...
                            if dstate:
                                ...
                                global_count.increment()
                                logging.debug(
                                    "{} processing DONE; ADDED kind: {}; new name: {}; G:{}".format(
                                        task_q.qsize(), "Job", job_name, global_count
                                    )
                                )
                                return  # donot remove this.
                            ...
                            global_count.increment()
                            logging.debug(
                                "{} processing DONE; Job is already {}. G:{}; ".format(
                                    task_q.qsize(), job_status, global_count
                                )
                            )
                        else:
                            logging.debug(
                                "{} The Job {} status is {}.- Trusting-1 Event Thread to do follow up actions".format(
                                    task_q.qsize(), job_name, job_status
                                )
                            )
                elif phase == "Failed":
                    logging.debug(
                        "{} The Build {} retriggered since status={}. G:{}".format(
                            task_q.qsize(), build_name, phase, global_count
                        )
                    )
                    trigger_build(
                    ...
                    logging.debug(
                        "{} The Build {} status is {}. TODO".format(
    ...
    # ==========================
        ...
        if not job_created:
            ...
            # JOB was created before the scheduler started.
            logging.debug(
                "{} The Job {} status is {}.Trusting Event Thread to do follow up actions.".format(
                    task_q.qsize(), q_resource_name, job_status
                )
            )

    else:
        logging.debug(
            "{} processing unknown resource : {}".format(
