...
def get_imagestream_endp(req_url, namespace, imagestream=None):
    if imagestream:
        return "{}/apis/image.openshift.io/v1/namespaces/{}/imagestreams/{}".format(
            req_url, namespace, imagestream
        )
    else:
        return "{}/apis/image.openshift.io/v1/namespaces/{}/imagestreams".format(
            req_url, namespace
...
def get_buildconfig_endp(req_url, namespace, build_config_name=None):
    if build_config_name:
        return "{}/apis/build.openshift.io/v1/namespaces/{}/buildconfigs/{}".format(
            req_url, namespace, build_config_name
        )
    else:
        return "{}/apis/build.openshift.io/v1/namespaces/{}/buildconfigs".format(
            req_url, namespace
...
def get_build_endp(req_url, namespace, build_name):
    return "{}/apis/build.openshift.io/v1/namespaces/{}/builds/{}".format(
        req_url, namespace, build_name
    )


def get_imagestream1(ocp_client, namespace, imagestream):
    try:
        response = ocp_client.resources.get(
...
def create_imagestream(req_url, req_headers, namespace, imagestream):
    ...
    response = requests.post(
        endpoint, json=imagestream, headers=req_headers, verify=False
    )
...
def create_buildconfig(req_url, req_headers, namespace, build_config):
    ...
    response = requests.post(
        endpoint, json=build_config, headers=req_headers, verify=False
    )
    logging.debug(
        "Status code for Buildconfig POST request: {}".format(response.json())
    )
...
def get_build_logs(req_url, req_headers, namespace, build_pod):
    build_pod_endpoint = "{}/api/v1/namespaces/{}/pods/{}/log".format(
        req_url, namespace, build_pod
    )
...
def get_status_build(req_url, req_headers, namespace, build_name):
    ...
    response = requests.get(endpoint, headers=req_headers, verify=False)
    logging.debug(
        "Status code for latest Build's GET request: {}".format(response.json())
    )
    if "status" in response.json():
        response = response.json()
        if "code" in response and response["code"] == 404:
